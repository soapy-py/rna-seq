...
adapted from jbagnall@broadinstitute.org, adapted from scripts by Marek Orzechowski</br>
Jan. 21, 2025</br>
...
vst_filepath = "/Users/sophie_chen/downloads/psa_rnaseq_manuscript_rproject-main/output/meow_vst_remove_low_count_samples_and_genes_n120x5849.gct"
...
#get rid of this if it doesn't work: assigning sample_meta$condition_id to col_meta$condition_id
col_meta <- merge(col_meta, sample_meta[, c("old_sample_id", "condition_id")], 
                  by.x = "original_cid", by.y = "old_sample_id", all.x = TRUE)

col_meta$condition_id <- col_meta$condition_id.y
col_meta <- col_meta[, !(names(col_meta) %in% c("condition_id.x", "condition_id.y"))]
