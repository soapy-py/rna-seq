...
adapted from jbagnall@broadinstitute.org</br>
Jan. 21, 2025</br>
...
## adding condition_id to cdesc_trim
```{r}
#taking condition_id from sample_meta  to cdesc_trim
cdesc_trim <- merge( cdesc_trim, sample_meta[, c("old_sample_id", "condition_id")],by.x = "original_cid", by.y = "old_sample_id")

#renaming condition-id.y and dropping extra cols
cdesc_trim$condition_id <- cdesc_trim$condition_id.y  
cdesc_trim <- cdesc_trim[, !(names(cdesc_trim) %in% c("condition_id.x", "condition_id.y"))]  

cdesc_trim$condition_id <- as.factor(cdesc_trim$condition_id)

rownames(cdesc_trim) <- cdesc_trim$original_cid
all(rownames(cdesc_trim) == colnames(cds_counts_trim))  # Should return true

#realigning and checking if true again
cdesc_trim <- cdesc_trim[match(colnames(cds_counts_trim), rownames(cdesc_trim)), ]
all(rownames(cdesc_trim) == colnames(cds_counts_trim))  # Should return true
